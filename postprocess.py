...
def plot_streamlines(output_dir="results"):
    ...
    # cs = ax.contour(X, Y, psi, levels=levels, cmap='RdBu_r', linewidths=0.8)
...
def plot_u_velocity_centerline(output_dir="results"):
    ...
    ax.set_title(
        "u-velocity profile at x = 0.5 (Re = 100)", fontsize=12, fontweight="bold"
    )
    ax.grid(True, alpha=0.3)
...
def plot_v_velocity_centerline(output_dir="results"):
    ...
    ax.set_title(
        "v-velocity profile at y = 0.5 (Re = 100)", fontsize=12, fontweight="bold"
    )
    ax.grid(True, alpha=0.3)
    ax.legend(loc="best", framealpha=0.9)
    ax.set_xlim([0, 1])
